<{ include file="cial/header.inc.tpl" }>
	<div class="comm">
		<{ cacheless }>
		<div class="dt"><div><b>评论列表</b>（评论：<a  href="article.php?aid=<{ $article.id }>"><{ $article.title }></a>）</div><span class="more">全部评论 (<span class="red"><{ $pageInfo.row_total }></span>)</span></div>	
	...
	<div class="comm">
		...
		<form action="comments.php?action=com" method="post">
			<span class="bai"><img src="<{ $stylePath }>/images/comm1.gif">&nbsp;请自觉遵守互联网相关的政策法规，严禁发布色情、暴力、反动的言论。
			...
			<input type="radio" name="cmt"  value="-1"><img alt="差评" src="<{ $stylePath }>/images/cmt-bad.gif">差评 		     </span>
			<{ $FCK }>
			<div class="subo"> 
				<input type="submit" class="button" name="sub" style="height:20px" value="发表评论">
				<span class="bai">&nbsp;&nbsp;网友评论仅供网友表达个人看法，并不表明本站同意其观点或证实其描述。</span>		
			</div>	
		</form>
		<{ else }>
			<span class="bai">登录后才能发表评论......</span>
		<{ /if }>
		<{ /cacheless }>
		<div class="nav"> </div>	
	</div>
